...
library(tidyr)
# Let us not remove aTrain <- drop_na(aTrain, LoanAmount, Loan_Amount_Term)
...
# prune the tree
classificationModel <- prune(classificationModel, cp = 0.008 ,"CP")
printcp(classificationModel)
fancyRpartPlot(classificationModel)
...
result <- data.frame(aTest$Loan_ID, test$Loan_Status)
View(result)
