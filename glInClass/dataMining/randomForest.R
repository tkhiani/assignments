...
## importing the data
RFDF.dev <- read.table("./glInClass/dataMining/DEV_SAMPLE.csv", sep = ",", header = T)
RFDF.holdout <- read.table("./glInClass/dataMining/HOLDOUT_SAMPLE.csv", sep = ",", header = T)
...
# Node size = 1% of the base, mtry = sqrt(no. of variables)
RF <- randomForest(as.factor(Target) ~ ., data = RFDF.dev[,-1], 
                   ntree=101, mtry = 5, nodesize = 50,
                   importance=TRUE)
